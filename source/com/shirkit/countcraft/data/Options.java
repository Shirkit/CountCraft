...
public class Options {
	...
	public static void load(FMLPreInitializationEvent event) {
		...
		BLOCK_TE_BUFFEREDENERGYCOUNTER = instance.config.get("BlockID", "block.buffered_te_energy_counter", BLOCK_TE_BUFFEREDENERGYCOUNTER).getInt();
		
		instance.config.addCustomCategoryComment("ItemID", "IDs for items (remember that they get shifted by 256 when loaded)");
		ITEM_BC_PIPEITEMCOUNTER = instance.config.get("ItemID", "item.pipe_item_counter", ITEM_BC_PIPEITEMCOUNTER).getInt();
		ITEM_BC_PIPEFLUIDCOUNTER = instance.config.get("ItemID", "item.pipe_fluid_counter", ITEM_BC_PIPEFLUIDCOUNTER).getInt();
